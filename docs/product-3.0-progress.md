...
The 3.1.4 implementation keeps the verified switch transaction from 3.0.x and all 3.1.0 authorization/storage boundaries: exact desktop quiesce, durable auth activation, same-package relaunch, official identity proof, atomic commit, verified rollback, four official Codex login methods, DPAPI last-known-good vault and SQLite schema v10. Manager never closes the installed Codex during automated release verification.
...
The exact clean 3.1.4 package contains 78/78 matching `dist`/ASAR files, exactly one referenced renderer JS and CSS entry, and no orphan historical bundles. Source/render/package evidence is recorded in `docs/releases/3.1.4.md`; native Windows hover timing and the staged 3.1.3→3.1.4 Sandbox upgrade remain explicit follow-up oracles.
