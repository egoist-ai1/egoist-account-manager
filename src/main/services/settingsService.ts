...
export class SettingsService {
  ...
  get(): AppSettings {
    ...
    try {
      const parsed = JSON.parse(raw) as Omit<Partial<AppSettings>, "autoRefreshIntervalMs" | "trayRefreshIntervalMs"> & {
      ...
      if (![0, 60_000, 180_000, 300_000, 600_000, 900_000].includes(settings.trayRefreshIntervalMs)) {
        settings.trayRefreshIntervalMs = defaultSettings.trayRefreshIntervalMs;
      }
      delete (settings as AppSettings & { desktopNotifications?: boolean }).desktopNotifications;
