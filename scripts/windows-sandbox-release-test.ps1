...
$installer313 = Join-Path $stageRoot "Codex-Account-Manager-Setup-3.1.3.exe"
$installer314 = Join-Path $stageRoot "Codex-Account-Manager-Setup-3.1.4.exe"
$portable314 = Join-Path $stageRoot "Codex-Account-Manager-3.1.4.exe"
...
$plan = [ordered]@{
  ...
  tests = @("fresh-install-3.1.3", "running-upgrade-3.1.4", "data-persistence", "uninstall", "reinstall-3.1.4", "portable-startup")
}
if (-not $Apply) {
...
foreach ($required in @($installer313, $installer314, $portable314)) {
...
try {
  Invoke-ProcessWithTimeout $installer313 @("/S") 120
  Wait-Until { Test-Path -LiteralPath $installedExe -PathType Leaf } 30 "3.1.3 executable was not installed."
  $results.checks.install313 = ((Get-Item -LiteralPath $installedExe).VersionInfo.ProductVersion -like "3.1.3*")

  $probe313 = Join-Path $env:TEMP "cam-sandbox-313"
  $results.checks.start313 = (Start-And-Probe $installedExe $probe313).rendererReady
  ...
  Wait-Until { @(Get-ManagedProcesses).Count -gt 0 } 20 "3.1.3 did not start before upgrade."
  Wait-Until { Test-Path -LiteralPath $databasePath -PathType Leaf } 20 "3.1.3 did not create accounts.sqlite."
  $preUpgradeProcessIds = @(Get-ManagedProcesses | Select-Object -ExpandProperty ProcessId)
  if ($preUpgradeProcessIds.Count -eq 0) { throw "No 3.1.3 process tree was captured before upgrade." }
  Invoke-ProcessWithTimeout $installer314 @("/S") 150
  Wait-Until {
    (Test-Path -LiteralPath $installedExe -PathType Leaf) -and ((Get-Item -LiteralPath $installedExe).VersionInfo.ProductVersion -like "3.1.4*")
  } 45 "Running upgrade did not install 3.1.4."
  try {
    Wait-Until {
      @($preUpgradeProcessIds | Where-Object { Get-Process -Id $_ -ErrorAction SilentlyContinue }).Count -eq 0
    } 30 "Installer did not close the captured 3.1.3 process tree."
  } catch { }
  ...
  Stop-ManagedProcesses
  $results.checks.runningUpgrade314 = $true
  ...
  Capture-Desktop (Join-Path $resultRoot "installed-3.1.4.png")
  Stop-ManagedProcesses
  ...
  Wait-Until { Test-Path -LiteralPath $installedExe -PathType Leaf } 30 "3.1.4 clean reinstall failed."
  $results.checks.reinstall314 = ((Get-Item -LiteralPath $installedExe).VersionInfo.ProductVersion -like "3.1.4*")
  ...
  $results.checks.portableStartup = (Start-And-Probe $portable314 $portableProbe 45).rendererReady
  $portableRoot = Split-Path -Parent $portable314
  foreach ($process in @(Get-CimInstance Win32_Process -ErrorAction SilentlyContinue | Where-Object {
    $_.ExecutablePath -and $_.ExecutablePath.StartsWith($portableRoot, [StringComparison]::OrdinalIgnoreCase) -and $_.Name -like "Codex-Account-Manager-3.1.4*"
  })) {
